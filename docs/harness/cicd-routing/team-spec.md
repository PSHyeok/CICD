...
```text
source adapter -> normalized event -> policy match -> atomic claim -> Jenkins
```
...
| Match | event and YAML policies | ordered matching policies | return no matches |
...
- Runtime reads the policy and secret environment variables, writes the SQLite
...
- Tests cover per-project path isolation and exact provider endpoint shape.
